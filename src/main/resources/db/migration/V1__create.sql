...
CREATE TABLE memos (id INT PRIMARY KEY AUTO_INCREMENT, title VARCHAR(30), text VARCHAR(30), username VARCHAR(100) NOT NULL DEFAULT 'user1', CONSTRAINT FK_MEMOS_USERNAME FOREIGN KEY (username) REFERENCES users (username));
INSERT INTO memos (title, text, username) VALUES ('買い物', '米', 'user1');
INSERT INTO memos (title, text, username) VALUES ('宿題', '数学', 'user1');